...
from data.locators import WatermaniacPageLocator
from data.data import *
from time import sleep


class WatermaniacPage(BasePage):

    def __init__(self, driver):
        self.locator = WatermaniacPageLocator
        self.data = Data
        self.watermaniacdata = Watermaniac
        super().__init__(driver)

    def get_current_day_status(self):

        current_day_percentage = self.get_text(self.locator.current_day_percentage)
        current_day_ml = self.get_text(self.locator.current_day_ml)
        current_day_remaining_ml = self.get_text(self.locator.current_day_remaining_ml)
        current_day_target_ml = self.get_text(self.locator.current_day_target_ml)
        curent_day_history = self.get_text(self.locator.curent_day_history)

        print('Current_day_percentage:', current_day_percentage)
        print('Current_day_ml:', current_day_ml)
        print('Current_day_remaining_ml:', current_day_remaining_ml)
        print('Current_day_target_ml:', current_day_target_ml)
        print('Current_day_history:', curent_day_history)
    ...
    #     # phone_number = self.read_excel_for_availability(self.wdata.file_for_availability, self.wdata.sheet_for_availability)
    #     phone_number = self.read_number_from_gspread(self.wdata.gsheet_name, self.wdata.gsheet_worksheet, 1)
    #     # print(phone_number)
    #     result = ''
    ...
    #             self.send_data(f'{self.wdata.country_code_BD}{int(phone_number[i])}', self.locator.search_input)  # For initial run
    ...
    #                 result = 'False'
    #             self.go_back()
    #             self.write_result_on_gspread(self.wdata.gsheet_name, self.wdata.gsheet_worksheet, i + 1, 2, result)  # For initial run
    #         except:
    #             continue
    ...
    #         try:
    #             self.click(self.locator.search)
    #             self.send_data(f'{self.wdata.country_code_BD}{int(phone_number[escaped_row[i] - 1])}', self.locator.search_input)  # For escaped value
    #             self.driver.hide_keyboard()
    #             try:
    #                 res = self.is_element_displayed(self.locator.chat_person)
    #                 if res is True:
    ...
    #                 result = 'False'
    #             self.go_back()
    #             self.write_result_on_gspread(self.wdata.gsheet_name, self.wdata.gsheet_worksheet, escaped_row[i], 2, result)  # For escaped value
    #         except:
    #             continue
    ...
    #
    # def send_image(self):
    #     available_phone_number = self.read_excel_for_availability(self.wdata.file_for_availability, self.wdata.sheet_for_availability)
    #     available_phone_number = self.get_wa_available_phone_number_gsheet(self.wdata.gsheet_name, self.wdata.gsheet_worksheet, 1, 2, 'TRUE')
